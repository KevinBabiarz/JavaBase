...
public class TestMap {
    ...
    private static Map<Integer, Set<Personne>> getParDizaine(Personne[] personnes) {
        Map<Integer, Set<Personne>> dizaineMap = new HashMap<>();

        for (Personne p : personnes) {
            Set<Personne> familleSet = dizaineMap.get(p.getAge()/10*10);
            if (familleSet == null) {
                Set<Personne> setPersonne = new TreeSet<>(getComparator());
                familleSet = new HashSet<>();
                dizaineMap.put(p.getAge()/10*10, familleSet);
    ...
    private static void affichageMap(Map<String, Set<Personne>> famille) {
        Iterator<Map.Entry<String, Set<Personne>>> entryIterator = famille.entrySet().iterator();
        while (entryIterator.hasNext()) {
            Map.Entry<String, Set<Personne>> next = entryIterator.next();
            System.out.println("nom = " + next.getKey() + " \nfamille = " + next.getValue());
        }
    }
